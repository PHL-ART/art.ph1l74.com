...
function CategoryPostRow({ post, index, currentCategorySlug }: CategoryPostRowProps) {
  ...
  return (
    ...
    >
      {/* Растянутая ссылка */}
      <Link href={`/post/${post.slug}`} className="absolute inset-0 z-[1]" aria-label={post.title} />
      ...
      <div className="flex sm:hidden flex-col justify-end relative overflow-hidden" style={{ height: '160px' }}>
        ...
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(to top, rgba(10,8,9,0.92) 40%, rgba(10,8,9,0.3) 100%)' }}
        />
        <div className="relative z-[2] pointer-events-none" style={{ padding: '12px 14px 14px' }}>
          {hasChips && (
            <CategoryChips categories={otherCategories} tags={post.tags} className="pointer-events-auto mb-[6px]" />
          )}
          <h2
      ...
      >
        {/* Столбец с датой */}
        <div className="flex-shrink-0 text-right relative z-[2] pointer-events-none" style={{ width: '48px' }}>
          {dateInfo ? (
            <>
              <div className="font-nav font-bold" style={{ fontSize: '22px', lineHeight: '1.0', letterSpacing: '-0.01em', color: 'var(--color-text)' }}>
                {dateInfo.day}
              </div>
              <div className="font-nav font-medium uppercase" style={{ fontSize: '10px', letterSpacing: '0.06em', color: 'var(--color-caption)', marginTop: '2px' }}>
                {dateInfo.month}
              </div>
              <div className="font-nav" style={{ fontSize: '10px', letterSpacing: '0.04em', color: 'var(--color-caption)', opacity: 0.45, marginTop: '1px' }}>
                {dateInfo.year}
              </div>
            </>
          ) : (
        ...
        <div className="relative z-[2] pointer-events-none" style={{ flex: 1, minWidth: 0 }}>
          {hasChips && (
            <CategoryChips categories={otherCategories} tags={post.tags} className="pointer-events-auto mb-[10px]" />
          )}
          <h2
            className="font-display font-bold lowercase group-hover:opacity-80 transition-opacity"
            style={{ fontSize: '26px', lineHeight: '1.05', letterSpacing: '-0.01em', color: 'var(--color-text)', margin: 0 }}
          >
            {post.title}
          </h2>
          {excerpt && (
            <p
              className="font-body"
              style={{ marginTop: '10px', fontWeight: 300, fontSize: '15px', lineHeight: '1.6', color: 'var(--color-caption)', maxWidth: '60ch', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}
            >
              {excerpt}
            </p>
          )}
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  return (
    <div style={{ background: 'var(--color-bg)' }}>
      ...
      {/* Заголовок раздела */}
      <div className="px-5 md:px-11" style={{ paddingTop: '52px', paddingBottom: '36px' }}>
        <h1
          className="font-display font-bold lowercase"
          style={{
      ...
      {/* Список постов */}
      <div className="px-5 md:px-11" style={{ paddingBottom: '64px' }}>
        {posts.length > 0 ? (
          <>
