...
interface Props {
  services: Service[]
  onAdd: (data: { name: string; slug: string; iconUrl?: string }) => Promise<void>
  onDelete: (id: string) => Promise<void>
}

export function AfterPostingBlock({ services, onAdd, onDelete }: Props) {
  ...
  async function handleAdd() {
    if (!name || !slug) return
    setAdding(true)
    await onAdd({ name, slug, iconUrl: iconUrl || undefined })
    setName('')
    setSlug('')
    setIconUrl('')
    setShowForm(false)
  ...
  const fields: [string, string, (v: string) => void][] = [
    ...
    ['Slug', slug, setSlug],
    ['URL иконки', iconUrl, setIconUrl],
  ]

  return (
    <div style={{ border: '1px solid rgba(255,255,255,0.1)', padding: 24 }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-display font-bold text-base m-0">AfterPosting</h2>
        <button
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {s.iconUrl && (
                ...
                <img src={s.iconUrl} alt={s.name} style={{ width: 18, height: 18, objectFit: 'contain' }} />
              )}
